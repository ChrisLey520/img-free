...
function getApiBase() {
  ...
  if (env) return env;
  return window.location.hostname === "localhost" ? "http://localhost:3002" : "/api";
}
...
export function RedeemContent() {
  ...
  async function handleValidateCode() {
    if (!code.trim()) { setCodeError("请输入兑换码"); return; }
    setBusy(true);
    setCodeError("");
    try {
      const res = await fetch(`${apiBase}/codes/validate`, {
        method: "POST",
      ...
      if (json.valid) {
        ...
        const buf = await fetch(`${apiBase}/results/${code.trim()}`);
        if (buf.ok) {
          const blob = await buf.blob();
          const url = URL.createObjectURL(blob);
          setResult({ previewDataUrl: url, expiresAt: json.expiresAt ?? "", alreadyDone: true });
          setStep("result");
        } else {
          setCodeError("兑换码已使用");
        }
        ...
        const msgs: Record<string, string> = {
          not_found: "兑换码不存在", used: "兑换码已使用", expired: "兑换码已过期",
        };
        setCodeError(msgs[json.reason ?? ""] ?? "兑换码无效");
      }
    } catch { setCodeError("网络错误，请重试"); }
  ...
  async function handleConvert() {
    if (!file) { setError("请先上传头像图片"); return; }
    setBusy(true);
    setError("");
    try {
      ...
      const json = (await res.json()) as {
        previewDataUrl?: string; inputPreviewDataUrl?: string;
        expiresAt?: string; alreadyDone?: boolean; message?: string;
      };
  ...
  function handleDownload() {
    ...
    a.href = `${apiBase}/results/${code.trim()}`;
    a.download = `pixel-avatar-${code.trim()}.png`;
    a.click();
  }

  return (
    <div className="flex flex-col items-center justify-start py-12 px-4 overflow-y-auto flex-1">
      <div className="w-full max-w-lg space-y-6">
        ...
        {step === "code" && (
          <Card>
            <CardHeader><CardTitle className="text-base">第 1 步：输入兑换码</CardTitle></CardHeader>
        ...
        {step === "upload" && (
          <Card>
            <CardHeader><CardTitle className="text-base">第 2 步：上传头像 & 选择尺寸</CardTitle></CardHeader>
            ...
            <CardContent className="space-y-4">
              <div className={`grid gap-4 ${result.inputPreviewDataUrl ? "grid-cols-2" : "grid-cols-1"}`}>
                ...
                <div className="text-center space-y-1">
                  ...
                  <img src={result.previewDataUrl} alt="像素图" className="mx-auto rounded border"
